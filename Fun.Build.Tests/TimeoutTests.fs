...
let ``timeout should work`` () =
    shouldBeCalled (fun call -> pipeline "timeout" {
        timeout 1
        stage "timeout" {
            run (
                async {
    ...
    Assert.Throws<PipelineCancelledException>(fun _ ->
        shouldNotBeCalled (fun call -> pipeline "timeout" {
            timeout 1
            stage "timeout" {
                run (
                    async {
    ...
    Assert.Throws<PipelineCancelledException>(fun _ -> pipeline "timeout" {
        timeout 2
...
let ``timeoutForStage should work`` () =
    Assert.Throws<PipelineFailedException>(fun _ -> pipeline "timeoutForStage" {
        timeoutForStage 1
...
let ``timeoutForStep should work`` () =
    Assert.Throws<PipelineFailedException>(fun _ ->
        shouldNotBeCalled (fun call -> pipeline "timeoutForStep" {
            timeoutForStep 1
...
let ``nested stage timeoutForStep should work`` () =
    Assert.Throws<PipelineFailedException>(fun _ -> pipeline "timeoutForStep" {
        timeoutForStep 1
    ...
    Assert.Throws<PipelineFailedException>(fun _ ->
        shouldBeCalled (fun call -> pipeline "timeoutForStep" {
            timeoutForStep 1
            stage "" {
                stage "" {
                    timeoutForStep 2
...
let ``nested stage timeoutForStage should work`` () =
    ...
    Assert.Throws<PipelineFailedException>(fun _ ->
        shouldBeCalled (fun call -> pipeline "timeoutForStage" {
            timeoutForStage 1
            stage "" {
                stage "" {
                    timeout 2
